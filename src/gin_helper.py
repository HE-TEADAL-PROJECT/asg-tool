...
def _get_transforms(transform_folder_path: str) -> list:
    ...
    """
    loaded_functions = files_helper.load_user_functions(folder_path=transform_folder_path)
...
def _add_ep_exports(
    ...
    ep_tr_instructs = [tr_instruct for tr_instruct in tr_instructs if tr_instruct["endpoint"] == ep_name]
    if ep_tr_instructs == []:
    ...
    for ep_tr_instructions in ep_tr_instructs:
        ...
        logger.debug(f"processing {len(api_calls)} api calls for {ep_name}.{output_df_name}.{target_name}")
        for api_call in api_calls:
            api_call_name = api_call.name
            api_call_params = api_call.parameters
            logger.debug(f"api_call_name={api_call_name}, api_call_params={api_call_params}")
            tool_details = _get_tool_for_call(api_call_name, ep_tr_instructions["context_metadata"])
            if not tool_details:
        ...
        exports["exports"][output_df]["fields"][target_name] = functions_param_section
...
def _get_prop_tr_instructs(
    ...
    gin_result = gin_tool_calling(
        ...
        query=gin_input,
        config_file=gin_config_path)
    if not gin_result:
        logger.warning(f"failed to generate tool call for property")
        return result
    ...
    if not gin_context or not len(gin_context):
        logger.warning(f"no tools context for property {prop_name} of element {el_name}")
        return result
    ...
    for sfdp_endpoint in sfdp_endpoints:
        for ep_name, ep_contents in sfdp_endpoint.items():
            ...
            for el_name, el_data in ep_schema.items():
                ...
                if not el_props:
                    logger.warning(f"element {el_name} in endpoint {ep_name} has no properties, skipping")
                    continue
...
def _get_ep_specs(
    ...
    result = {}
    logger.debug(f"enter for {len(endpoints)} endpoints and {len(tr_instructs)} transforms")
    ...
    for endpoint in endpoints:
        ...
        full_ep_spec = _add_ep_exports(
            ep_name, ep_spec, tr_instructs
        )
...
def generate_sfdp(
    ...
    logger.debug(f"loaded fdp_spec")
    asg_spec = openapi_helper.load_openapi_spec(instructions_path)
    logger.debug(f"loaded asg_spec")
    ...
    logger.debug(f"will generate {len(endpoints)} endpoints:\n{pformat(endpoints, sort_dicts=False)}")
